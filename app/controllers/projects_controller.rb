class ProjectsController < ApplicationController
  ...
  def create_grade clss, params, user_id, project_id
  ...
  def submit_grade
    ...
    if(can?(:grade_cost, @project) and params[:cost_score])
      create_grade(CostScore,params[:cost_score],@user.id, @project.id)
    end
    if(can?(:grade_technology, @project) and params[:technology_score])
      create_grade(TechnologyScore,params[:technology_score],@user.id, @project.id)
    end
    if(can?(:grade_development, @project) and params[:development_score])
      create_grade(DevelopmentScore,params[:development_score],@user.id, @project.id)
    end
    if(can?(:grade_legal, @project) and params[:legal_score])
      create_grade(LegalScore,params[:legal_score],@user.id, @project.id)
    end
    if(can?(:grade_risks, @project) and params[:risk_score])
      create_grade(RiskScore,params[:risk_score],@user.id, @project.id)
    end
    if(can?(:grade_opportunities, @project) and params[:opportunity_score])
      create_grade(OpportunityScore,params[:opportunity_score],@user.id, @project.id)
    end
